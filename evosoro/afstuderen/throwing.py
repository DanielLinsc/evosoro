...
from evosoro.tools.algorithms import ParetoOptimizationReset #ParetoOptimization #AnnealingOptimization, ParetoOptimizationDiversifyAncestry,  #, ParetoTournamentOptimization
...
IND_SIZE = (20,10,3)  # Bounding box dimensions (x,y,z). e.g. IND_SIZE = (6, 6, 6) -> workspace is a cube of 6x6x6 voxels 20 12 2
...
MAX_EVAL_TIME = 41  #60 (seconds) wait this long exitbefore giving up on evaluating this individual
SAVE_LINEAGES = True
...
RUN_DIR = "throwing1_data".format(SEED)  # Subdirectory where results are going to be generated
RUN_NAME = "throwing1{}".format(SEED)
...
class MyGenotype(Genotype):
    def __init__(self):
        ...
        self.to_phenotype_mapping.add_map(name="material", tag="<Data>", func=make_material_tree,
                                          dependency_order=["shape", "muscleOrTissue"], output_type=int)  # BUGFIX: "tissueType" was not listed
        ...
        #self.to_phenotype_mapping.add_output_dependency(name="muscleType", dependency_name="muscleOrTissue",
         #                                               requirement=True, material_if_true="3", material_if_false="4")#3,4


# Define a custom phenotype, inheriting from the Phenotype class
class MyPhenotype(Phenotype):
    def is_valid(self, min_percent_full=0.01, min_percent_muscle=0.0, max_percent_full=1):
...
#my_objective_dict.add_objective(name="PushDist", maximize=False, tag="")
...
my_objective_dict.add_objective(name="num_voxels", maximize=False, tag=None,
                                node_func=np.count_nonzero, output_node_name="material", logging_only=True)
...
my_optimization = ParetoOptimizationReset(my_sim, my_env, my_pop)
