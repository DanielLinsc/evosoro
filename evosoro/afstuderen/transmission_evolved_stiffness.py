...
SEED = 8#int(sys.argv[1])
random.seed(SEED)  # Initializing the random number generator for reproducibility
np.random.seed(SEED)


RUN_DIR = "transmission_stiffness_{}_data".format(SEED)  # Subdirectory where results are going to be generated
RUN_NAME = "transmission_stiffness_{}".format(SEED)
...
class MyGenotype(Genotype):
    def __init__(self):
        ...
        self.to_phenotype_mapping.add_map(name="material", tag="<Data>", func=make_material_tree,
                                          dependency_order=["shape"], output_type=int)  # BUGFIX: "tissueType" was not listed

        self.to_phenotype_mapping.add_output_dependency(name="shape", dependency_name=None, requirement=None,
                                                        material_if_true=None, material_if_false="1")
