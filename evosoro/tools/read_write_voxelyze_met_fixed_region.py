...
def write_voxelyze_file(sim, env, individual, run_directory, run_name):
    ...
    env.hurdle_height = 2
    env.num_hurdles = 5
    env.space_between_hurdles = 1
    env.circular_hurdles = False
    env.tunnel_width = 1
    env.forward_hurdles_only = False
    env.needle_position = 0.01
    env.wall_height = 2
...
#tot hier
    ...
    for name, tag in env.new_param_tag_dict.items():
        ...
        # geplakte stuk uit nieuwe script vanaf hier
        ...
        for z in range(individual.genotype.orig_size_xyz[2]):
            ...
            for y in range(individual.genotype.orig_size_xyz[1]):
                for x in range(individual.genotype.orig_size_xyz[0]):
                    voxelyze_file.write("7")  # was 3, aangepast
