...
#from read_write_voxelyze_single_needle import read_voxlyze_results, write_voxelyze_file
...
def evaluate_all(sim, env, pop, print_log, save_vxa_every, run_directory, run_name, max_eval_time,
    ...
    for ind in pop:
        ...
        # otherwise evaluate with voxelyze
            ...
            #if sshcon == True:
            # DEZE sub.Popen("ssh " + node[ind.id%number_nodes] + " \"cd evosoro/evosoro/afstuderen && ./voxelyze  -f " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%05i.vxa\"" % ind.id,
                 #   shell=True)
            #else:
            #if sshcon == True:
